...
export default function ProfileClient({ data }: { data: ProfileData }) {
  ...
  const umkmFields = [
    "name","alamat","rt_rw","desa_kel","kecamatan","kabupaten","provinsi",
    "tahunBerdiri","usahaUtama","produkUtama","badanHukum","jumlahKaryawan",
    "sistemPenjualan","persaingan","totalAset","penjualanPerTahun",
    "proyeksiPenjualan","nilaiAsetJaminan","jumlahDokumenKredit","description",
    "noRekening","noNPWP","noBIP",
  ];
    ...
    <div className="p-6 max-w-3xl mx-auto space-y-8">
      ...
      <section className="bg-white p-6 shadow rounded">
        <div className="flex justify-between items-center mb-4">
          ...
          <button onClick={() => setOpenProfile(true)} className="text-blue-600 hover:underline">Edit</button>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[
            ...
            <div key={label} className="flex flex-col">
              ...
              <input type="text" value={String(val)} disabled className="mt-1 p-2 border rounded bg-gray-100" />
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="bg-white p-6 shadow rounded">
        <div className="flex justify-between items-center mb-4">
          ...
          <button onClick={() => setOpenUmkm(true)} className="text-blue-600 hover:underline">Edit</button>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
